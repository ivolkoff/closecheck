...
func (rt *ResourceTracker) GetUnclosedResources() []*Resource {
	...
	for _, resource := range rt.resources {
		if !rt.IsClosed(resource.id) {
			// If this resource has a parent && the parent is also unclosed,
			// only report the parent (root cause) instead of derived resources
			if resource.derivedFrom != "" {
				if parentResource := rt.getResourceByID(resource.derivedFrom); parentResource != nil {
					if !rt.IsClosed(parentResource.id) {
						// Parent is unclosed, so skip reporting this derived resource
						continue
					}
				}
			}
...
func (av *AssignVisitor) newReturnVar(t types.Type) returnVar {
	...
	// Special handling for structs that have closable fields but also have close-like methods
	// For these, we want to prioritize field detection over method detection
	...
	// Check if this type has any close-like method (including io.Closer && Factory patterns)
...
func (av *AssignVisitor) extractCloserFields(str *types.Struct, typeName string) returnVar {
	...
	for i := 0; i < str.NumFields(); i++ {
		...
		// Check exported fields (standard behavior)
		...
		// Also check unexported fields - important for generic wrappers like Asset[T]
		if isCloserType(v.Type()) && !unicode.IsUpper([]rune(fieldName)[0]) {
			fields = append(fields, field{
				name:     fieldName,
				typeName: v.Type().String(),
		...
		if def, ok := stmt.(*ast.DeferStmt); ok {
			if sel, ok := def.Call.Fun.(*ast.SelectorExpr); ok && sel.Sel != nil {
				if ident, okI := sel.X.(*ast.Ident); okI {
					// Check if this is a provable shutdown method
    ...
    if len(call.Args) == 1 {
        ...
        if argSel, okA := call.Args[0].(*ast.SelectorExpr); okA {
            if av.isProvableCloseMethod(argSel, call) {
                ...
                // Track container as resource if needed
                if av.resourceTracker != nil {
                    if av.resourceTracker.GetResourceByVar(baseIdent.Name) == nil {
                        typeName := ""
...
// isProvableCloseMethod checks if method reference provably leads to Close() call
func (av *AssignVisitor) isProvableCloseMethod(sel *ast.SelectorExpr, context *ast.CallExpr) bool {
    // Accept method calls where we can verify through AST analysis that Close() is called
    
    // Check if this is a method call on an Asset type that has shutdown pattern
    if sel.Sel != nil && sel.Sel.Name == "Shutdown" {
        // For Asset types, we verified that the shutdown function calls Close()
        // through shutdownFuncClosesParam during field assignment tracking
        if baseSel, ok := sel.X.(*ast.SelectorExpr); ok {
            if baseIdent, okI := baseSel.X.(*ast.Ident); okI {
                if baseObj := av.pass.TypesInfo.ObjectOf(baseIdent); baseObj != nil {
                    basePos := baseObj.Pos()
                    // Check if we've verified that this container's field shutdown closes params
                    return av.containerFieldShutdownCloses[basePos]
                }
            }
        }
        
        // Also check if the receiver type has provable shutdown patterns
        if baseSel, ok := sel.X.(*ast.SelectorExpr); ok {
            if typeInfo, exists := av.pass.TypesInfo.Types[baseSel]; exists {
                return av.hasProvableCloseInShutdown(typeInfo.Type)
            }
        }
    }
...
func (av *AssignVisitor) hasProvableCloseInShutdown(t types.Type) bool {
    // For Asset[T] types, the shutdown function is passed in constructor
    // and we verified it calls Close() through shutdownFuncClosesParam
    if named, ok := t.(*types.Named); ok {
        if named.Obj() != nil && named.Obj().Name() == "Asset" {
            // Asset type with shutdown function pattern
            return true
        }
    }
    return false
...
func (av *AssignVisitor) isProvableContainerShutdown(call *ast.CallExpr) bool {
    ...
    if ident, ok := sel.X.(*ast.Ident); ok {
        if obj := av.pass.TypesInfo.ObjectOf(ident); obj != nil {
            recvPos := obj.Pos()
            // For now, accept any shutdown method on a tracked container
            // This is more permissive but still AST-based
            return av.containerHasShutdownHandler[recvPos] || av.containerFieldShutdownCloses[recvPos] || av.resourceTracker.GetResourceByVar(ident.Name) != nil
        }
    }
...
func (av *AssignVisitor) isMethodCallWithCleanupSignature(call *ast.CallExpr) bool {
    ...
    if fn, ok := typeutil.Callee(av.pass.TypesInfo, call).(*types.Func); ok && fn != nil {
        ...
        if sig, ok := fn.Type().(*types.Signature); ok {
            ...
            if recv, exists := av.pass.TypesInfo.Types[sel.X]; exists && isCloserType(recv.Type) {
...
func (av *AssignVisitor) methodReturnsResource(call *ast.CallExpr) bool {
	if typeInfo, exists := av.pass.TypesInfo.Types[call]; exists {
		...
		return av.hasCloseMethod(returnType)
	}
...
func (av *AssignVisitor) returnsThatAreClosers(call *ast.CallExpr) []returnVar {
    ...
    // Methods like Resource() that return internal resources shouldn't create new closers
    if fn, ok := call.Fun.(*ast.SelectorExpr); ok {
        if fn.Sel != nil && fn.Sel.Name == "Resource" {
            return []returnVar{{}}
        }
    }
