...
func (rt *ResourceTracker) GetUnclosedResources() []*Resource {
	// First pass: identify parents that have unclosed children
	parentsWithUnclosedChildren := make(map[ResourceID]bool)
	for _, res := range rt.resources {
		if res.hasCloseMethod && !rt.IsClosed(res.id) && res.derivedFrom != "" {
			if parent := rt.getResourceByID(res.derivedFrom); parent != nil {
				if !rt.IsClosed(parent.id) {
					parentsWithUnclosedChildren[parent.id] = true
				}
			}
		}
	}
	...
	for _, resource := range rt.resources {
		if rt.IsClosed(resource.id) {
			continue
		}
		// Report parents that own unclosed children, even if they aren't closers themselves
		if parentsWithUnclosedChildren[resource.id] {
			unclosed = append(unclosed, resource)
			continue
		}
		// Otherwise report unclosed closers, suppressing if their parent is unclosed (closer or not)
		if resource.hasCloseMethod {
			if resource.derivedFrom != "" {
				if parent := rt.getResourceByID(resource.derivedFrom); parent != nil {
					if !rt.IsClosed(parent.id) {
						continue
					}
				}
...
func (av *AssignVisitor) traverse(stmts []ast.Stmt) bool {
							...
							if baseIdent, okB := sel.X.(*ast.Ident); okB {
								...
								if av.resourceTracker != nil {
									if parentRes := av.resourceTracker.GetResourceByVar(baseIdent.Name); parentRes != nil {
										...
										// hasCloseMethod=true to ensure reporting if not closed later
										av.resourceTracker.TrackResourceCreation(baseIdent.Name, typeName, call, basePos, true)
									}
								}
							}
						}
					...
					if sel, okS := castedStmt.Lhs[0].(*ast.SelectorExpr); okS {
						if baseIdent, okB := sel.X.(*ast.Ident); okB {
							if av.shutdownFuncClosesParam(castedStmt.Rhs[0]) {
								// Map base container var pos -> true
								basePos := baseIdent.Pos()
								if bObj := av.pass.TypesInfo.ObjectOf(baseIdent); bObj != nil {
									basePos = bObj.Pos()
								}
							...
							// Detect slice-append registration: a.shutdownHandlers = append(a.shutdownHandlers, a.DB.Shutdown)
							if call, ok := castedStmt.Rhs[0].(*ast.CallExpr); ok {
								if ident, okI := call.Fun.(*ast.Ident); okI && ident.Name == "append" && len(call.Args) >= 2 {
									// First arg must be same selector as LHS (a.shutdownHandlers)
									if argSel, okArgSel := call.Args[0].(*ast.SelectorExpr); okArgSel {
										if lhsBase, okLB := sel.X.(*ast.Ident); okLB {
											if argBase, okAB := argSel.X.(*ast.Ident); okAB && av.sameObject(lhsBase, argBase) && sel.Sel.Name == argSel.Sel.Name {
												// Any additional arg of form a.<Field>.Shutdown registers a handler
												for _, aexp := range call.Args[1:] {
													if shSel, okSh := aexp.(*ast.SelectorExpr); okSh && shSel.Sel != nil && shSel.Sel.Name == "Shutdown" {
														// Check base is selector on same container: a.DB
														if shBaseSel, okB2 := shSel.X.(*ast.SelectorExpr); okB2 {
															if shBaseIdent, okBI2 := shBaseSel.X.(*ast.Ident); okBI2 && av.sameObject(lhsBase, shBaseIdent) {
																basePos := lhsBase.Pos()
																if bObj := av.pass.TypesInfo.ObjectOf(lhsBase); bObj != nil {
																	basePos = bObj.Pos()
																}
																av.containerHasShutdownHandler[basePos] = true
															}
	...
	for _, stmt := range stmts {
		...
		// Track deferred container shutdowns where we can prove Close() is called
		if def, ok := stmt.(*ast.DeferStmt); ok {
			if sel, ok := def.Call.Fun.(*ast.SelectorExpr); ok && sel.Sel != nil {
				if ident, okI := sel.X.(*ast.Ident); okI {
					// Check if this is a provable shutdown method on a container
					if av.isProvableContainerShutdown(def.Call) {
						// Mark the container as closed in the tracker
						if av.resourceTracker != nil {
							av.resourceTracker.TrackResourceClosure(ident.Name, stmt)
						}
						// Additionally, mark any variables obtained from this container as closed
						// This covers cases where position-based mapping failed but name mapping exists
						for _, tgt := range posListToClose {
							if av.variableObtainedFromFactory(tgt.name, ident.Name) {
								tgt.wasClosedOrReturned = true
								if av.resourceTracker != nil {
									av.resourceTracker.TrackResourceClosure(tgt.name, stmt)
								}
							}
						}
					}
	...
	// Fallback aggregation: if a resource was obtained from a factory/container
	// and that container is unclosed, prefer reporting the container instead of the child.
	finalResources := make([]*Resource, 0, len(unclosedResources))
	seen := make(map[ResourceID]bool)
	for _, resource := range unclosedResources {
		parentName, fromFactory := av.factoryVariables[resource.varName]
		if fromFactory && av.resourceTracker != nil {
			if parentRes := av.resourceTracker.GetResourceByVar(parentName); parentRes != nil {
				if !av.resourceTracker.IsClosed(parentRes.id) {
					if !seen[parentRes.id] {
						finalResources = append(finalResources, parentRes)
						seen[parentRes.id] = true
					}
					// Skip reporting the child when parent is unclosed
					continue
				}
			}
		}
...
func (av *AssignVisitor) typeKeyForExpr(expr ast.Expr) string {
	...
	if tv, ok := av.pass.TypesInfo.Types[expr]; ok {
		return av.typeKey(tv.Type)
	}
	if id, ok := expr.(*ast.Ident); ok {
		if obj := av.pass.TypesInfo.ObjectOf(id); obj != nil {
			return av.typeKey(obj.Type())
		}
	}
	return ""
}
...
func (av *AssignVisitor) heavyProjectHasShutdownCall(recvTypeKey string) bool {
    ...
    if err != nil || packages.PrintErrors(pkgs) > 0 {
        av.heavyScanCacheMu.Lock()
        av.heavyScanCache[cacheKey] = false
        av.heavyScanCacheMu.Unlock()
        return false
    }
    ...
    for _, pkg := range pkgs {
        ...
        for _, file := range pkg.Syntax {
            ...
            ast.Inspect(file, func(n ast.Node) bool {
                if found || n == nil { return false }
                // Prune subtrees that cannot contain calls (dead ends)
                switch n.(type) {
                case *ast.ImportSpec, *ast.StructType, *ast.InterfaceType, *ast.FuncType, *ast.FieldList:
                    return false
                }
                ...
                if !ok || fn == nil { return true }
                sig, ok := fn.Type().(*types.Signature)
                if !ok || sig.Recv() == nil { return true }
                if av.typeKey(sig.Recv().Type()) != recvTypeKey { return true }

                // Accept explicit Close() calls on the container
                if sel.Sel.Name == "Close" {
                    found = true
                    return false
                }
                // Otherwise, verify method body matches shutdown pattern in its defining package
                if isShutdownMethodByPkgLoad(fn) {
                    found = true
                    return false
                }
                return true
...
// isShutdownMethodDecl checks if the given function is a shutdown method by AST pattern
func isShutdownMethodDecl(pkg *packages.Package, fn *types.Func) bool {
    if fn == nil { return false }
    fnName := fn.Name()
    // Build a normalized receiver key for fn (pkgpath.TypeName without pointer)
    var fnRecvKey string
    if sig, ok := fn.Type().(*types.Signature); ok && sig.Recv() != nil {
        // Reuse local helper logic: pkg path + named name without pointer
        recvType := sig.Recv().Type()
        if p, ok := recvType.(*types.Pointer); ok { recvType = p.Elem() }
        if n, ok := recvType.(*types.Named); ok && n.Obj() != nil && n.Obj().Pkg() != nil {
            fnRecvKey = n.Obj().Pkg().Path() + "." + n.Obj().Name()
        } else {
            fnRecvKey = recvType.String()
        }
    }
    for _, file := range pkg.Syntax {
        for _, decl := range file.Decls {
            ...
            // Prefer exact object identity when available
            if obj, ok := pkg.TypesInfo.Defs[fd.Name]; ok && obj == fn {
                if methodDeclLooksLikeShutdown(fd) { return true }
                continue
            }
            // Fallback: match by name and normalized receiver type in the same package
            if fd.Name.Name != fnName || fd.Recv == nil { continue }
            // Try to compute receiver key from types info on receiver list
            var recvKey string
            // Take first receiver field's type
            if len(fd.Recv.List) > 0 {
                if tv, ok := pkg.TypesInfo.Types[fd.Recv.List[0].Type]; ok {
                    recvT := tv.Type
                    if p, ok := recvT.(*types.Pointer); ok { recvT = p.Elem() }
                    if n, ok := recvT.(*types.Named); ok && n.Obj() != nil && n.Obj().Pkg() != nil {
                        recvKey = n.Obj().Pkg().Path() + "." + n.Obj().Name()
                    } else {
                        recvKey = recvT.String()
                    }
                }
            }
            if fnRecvKey != "" && recvKey != "" && fnRecvKey == recvKey {
                if methodDeclLooksLikeShutdown(fd) { return true }
            }
        }
    }
...
func methodDeclLooksLikeShutdown(fd *ast.FuncDecl) bool {
    if fd == nil || fd.Recv == nil || fd.Body == nil { return false }
    recvName := ""
...
// isShutdownMethodByPkgLoad loads the function's defining package and checks the method body
func isShutdownMethodByPkgLoad(fn *types.Func) bool {
    if fn == nil || fn.Pkg() == nil { return false }
    cfg := &packages.Config{Mode: packages.NeedSyntax | packages.NeedTypes | packages.NeedTypesInfo}
    pkgs, err := packages.Load(cfg, fn.Pkg().Path())
    if err != nil || len(pkgs) == 0 || packages.PrintErrors(pkgs) > 0 {
        return false
    }
    for _, p := range pkgs {
        if isShutdownMethodDecl(p, fn) {
            return true
        }
    }
    return false
...
// based on inter-procedural facts (containerMethodFact) and, as a fallback, by loading the
// defining package and analyzing the method body (isShutdownMethodByPkgLoad). No name-based
// heuristics are used.
	...
	if ident, ok := sel.X.(*ast.Ident); ok {
		if obj := av.pass.TypesInfo.ObjectOf(ident); obj != nil {
            ...
            if fndecl, _ := typeutil.Callee(av.pass.TypesInfo, call).(*types.Func); fndecl != nil {
                ...
                if isShutdownMethodByPkgLoad(fndecl) {
                    return true
                }
            }
