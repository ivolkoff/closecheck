...
func (av *AssignVisitor) heavyProjectHasShutdownCall(recvTypeKey string) bool {
	...
	for _, pkg := range pkgs {
		...
		for _, file := range pkg.Syntax {
			...
			// Skip vendor files when vendor is trusted
			if trustVendor {
				if pos := pkg.Fset.Position(file.Pos()); isVendorFilename(pos.Filename) {
					continue
				}
			}
			ast.Inspect(file, func(n ast.Node) bool {
				...
				if av.typeKey(sig.Recv().Type()) != recvTypeKey {
					return true
				}
...
func isShutdownMethodDecl(pkg *packages.Package, fn *types.Func) bool {
	...
	for _, file := range pkg.Syntax {
		for _, decl := range file.Decls {
			...
			if fd.Name.Name != fnName || fd.Recv == nil {
				continue
			}
...
func isShutdownMethodByPkgLoad(fn *types.Func) bool {
	if cached, ok := isShutdownMethodCached(fn); ok {
		return cached
	}
	if !enablePerMethodPkgLoad {
		cacheShutdownMethod(fn, false)
		return false
	}
	...
	isShutdown := false
	for _, p := range pkgs {
		// If all files of the package are in vendor and vendor is trusted,
		// skip deep analysis
		if trustVendor {
			allVendor := true
			for _, f := range p.Syntax {
				if pos := p.Fset.Position(f.Pos()); !isVendorFilename(pos.Filename) {
					allVendor = false
					break
				}
			}
	...
	cacheShutdownMethod(fn, isShutdown)
	return isShutdown
}
